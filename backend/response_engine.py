...
def generate_response(
    ...
    if recent_emotions and len(recent_emotions) >= 3:
        ...
        if dominant == emotion and emotion in ("sadness", "anxiety", "anger"):
            parts.append("")
            parts.append(
                f"*I've noticed you've been feeling {emotion} quite a bit recently. "
                f"It might help to talk to someone you trust about what's been going on.*"
            )

    # ── 5. Coping strategy (skip for happiness) ──────────────────────────────
    if emotion != "happiness":
        strategy_pool = COPING_STRATEGIES.get(emotion, COPING_STRATEGIES["neutral"])
        strategy = random.choice(strategy_pool)
        parts.append("")
        parts.append("Here's something that might help right now:")
        parts.append(f"> {strategy}")
    else:
        strategy_pool = COPING_STRATEGIES["happiness"]
        parts.append("")
        parts.append(random.choice(strategy_pool))
